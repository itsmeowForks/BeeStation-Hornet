...
const Stamp = (props, context) => {
  const { image, opacity, ...rest } = props;
  ...
  return <div className={classes(['paper121x54', image.sprite])} style={stamp_trasform} />;
};
...
const PaperSheetView = (props, context) => {
  ...
  return (
    <Box position="relative" backgroundColor={backgroundColor} width="100%" height="100%">
      <Box fillPositionedParent width="100%" height="100%" dangerouslySetInnerHTML={text_html} p="10px" />
      {stamp_list.map((o, i) => (
...
class PaperSheetStamper extends Component {
  ...
  findStampPosition(e) {
    ...
    const pos = [position.x - offset.left, position.y - offset.top];
  ...
  render() {
    const { value, stamp_class, stamps, ...rest } = this.props;
    const stamp_list = stamps || [];
    const current_pos = {
      sprite: stamp_class,
      x: this.state.x,
    ...
    return (
      <Box
        ...
        <Stamp opacity={0.5} image={current_pos} />
      </Box>
    );
  }
}
...
class PaperSheetEdit extends Component {
  ...
  createPreview(value, do_fields = false) {
    ...
    const { text, pen_color, pen_font, is_crayon, field_counter, edit_usr } = data;
    const out = { text: text };
    ...
    if (value.length > 0) {
      ...
      const fielded_text = createFields(signed_text, pen_font, 12, pen_color, field_counter);
      ...
      const fonted_text = setFontinText(formated_text, pen_font, pen_color, is_crayon);
    ...
    if (do_fields) {
      ...
      const final_processing = checkAllFields(out.text, pen_font, pen_color, edit_usr, is_crayon);
  ...
  render() {
    const { value = '', textColor, fontFamily, stamps, backgroundColor, ...rest } = this.props;
    return (
      <Flex direction="column" fillPositionedParent>
        <Flex.Item>
          <Tabs>
              ...
              onClick={() =>
                this.setState(() => {
                  const new_state = {
                    ...
                    combined_text: this.createPreview(this.state.textarea_text).text,
                  };
                  return new_state;
                })
              }>
            ...
            <Tabs.Tab
              ...
              textColor={'black'}
              backgroundColor={
                this.state.previewSelected === 'confirm' ? 'red' : this.state.previewSelected === 'save' ? 'grey' : 'white'
              }
        ...
        <Flex.Item grow={1} basis={1}>
          {(this.state.previewSelected === 'Edit' && (
            ...
            <PaperSheetView value={this.state.combined_text} stamps={stamps} fontFamily={fontFamily} textColor={textColor} />
          )}
        </Flex.Item>
        {this.state.showingToolTip === 1 && this.helpText()}
      </Flex>
    );
  }
  ...
  helpText() {
    return (
      <Box position="absolute" left="10px" top="25px" width="300px" height="385px" backgroundColor="#E8E4C9" textAlign="center">
        ...
        <Table>
          ...
          <Table.Row>
            <Table.Row>[_____]</Table.Row>
            <Table.Cell>Input field</Table.Cell>
          </Table.Row>

          <Table.Row>
            <Table.Cell>{centerText}</Table.Cell>
            <Table.Cell>
...
export const PaperSheet = (props, context) => {
  ...
  const { edit_mode, text, paper_color, pen_color = 'black', pen_font = 'Verdana', stamps, stamp_class, stamped } = data;
  // You might ask why?  Because Window/window content do weird
  // css stuff with white for some reason
  const backgroundColor = paper_color && paper_color !== 'white' ? paper_color : '#FFFFFF';
  ...
  const decide_mode = (mode) => {
    switch (mode) {
      ...
      case 2:
        return <PaperSheetStamper value={text} stamps={stamp_list} stamp_class={stamp_class} />;
      default:
        return 'ERROR ERROR WE CANNOT BE HERE!!';
    }
  };
  return (
    <Window theme="paper" width={400} height={500}>
      <Window.Content>
        <Box fillPositionedParent backgroundColor={backgroundColor}>
