...
/obj/structure/puzzle_element/Move(atom/newloc, direct, update_dir = TRUE, glide_size_override = 0)
	if(!isturf(newloc) ||  moving_diagonally || get_dist(get_step(src,direct),get_turf(source)) > 1)
		return 0
	else
		return ..()
